...
(MENU, SELECT_JOB, SELECT_REGION, SELECT_DISTRICT, SELECT_BRANCH, NAME, PHONE,
 ADMIN_NAV, ADMIN_INPUT, ADMIN_DELETE_CONFIRM, ADMIN_LINK_JOB) = range(11)
...
def navigation_kb(items, level_name):
    # Dynamic buttons for items + Management buttons
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📋 Available Jobs":
        jobs = db_query("SELECT id, title FROM jobs")
        if not jobs:
            await update.message.reply_text("No jobs.")
            return MENU
        kb = [[j[1]] for j in jobs]
        await update.message.reply_text("Select Position:", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))
        return SELECT_JOB
    elif text == "🛠 Admin Panel" and update.effective_user.id in ADMIN_IDS:
        return await admin_nav_regions(update, context)
    return MENU

# (User flow job_selected, region_selected etc. omitted for brevity, logic remains same as v3)
...
async def admin_nav_regions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📍 Admin: Select Viloyat:", reply_markup=navigation_kb(regions, "Viloyat"))
    return ADMIN_NAV
...
async def admin_nav_districts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🏙 Admin: Districts in {context.user_data['region_name']}:", 
                                   reply_markup=navigation_kb(districts, "District"))
    return ADMIN_NAV
...
async def admin_nav_branches(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🏪 Admin: Branches in {context.user_data['district_name']}:", 
                                   reply_markup=navigation_kb(branches, "Branch"))
    return ADMIN_NAV
...
async def admin_nav_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🏠 Main Menu": return await start(update, context)
    ...
    if text.startswith("➕ Add"):
        await update.message.reply_text(f"Enter the name for new {level}:", reply_markup=ReplyKeyboardRemove())
        return ADMIN_INPUT

    # DELETE Logic
    if text.startswith("❌ Delete"):
        table = "regions" if level == 'region' else "districts" if level == 'district' else "branches"
        # Filter items based on parent if necessary
        ...
        kb.append(["⬅️ Back"])
        await update.message.reply_text(f"Select which {level} to DELETE:", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))
        return ADMIN_DELETE_CONFIRM

    # Selection Logic (Moving down the tree)
    if level == 'region':
...
async def admin_delete_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Deleted {item_name}")
...
def main():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            # (Other User States: SELECT_JOB, etc. should be mapped here)
        },
